...
public:
    ...
    int authorize_encrypted(const char* passwd);
    int exchange_versions(VERSION_INFO&);
    int get_state(CC_STATE&);
    int get_results(RESULTS&, bool active_only = false);
    int get_results_CG(RESULTS&, bool bCpu = true, bool bGpu = true, bool active_only = false);
    ...
    int run_benchmarks();
    int set_proxy_settingsV6(GR_PROXY_INFO&);
   ...
   // int get_cc_config_v6(CONFIG_V6& config, LOG_FLAGS_V6& log_flags);
   // int set_cc_config_v6(CONFIG_V6& config, LOG_FLAGS_V6& log_flags);
